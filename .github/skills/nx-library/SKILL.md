...
`<client>` is the owning client's short name, matching its `apps/<client>/` folder; `<domain>` is the feature slug within that client's context.

**Note:** this workspace has no libraries yet (no `libs/` directory) — this is the intended convention for the first one, not an observed pattern. Revisit this file once a few real libs exist, in case actual usage diverges from the plan below.
...
- `store/` (feature lib) or `stores/` (shared lib) — state store `*.store.ts` + composable `with-*.feature.ts`; specs under `store/specs/`, test doubles under `store/tests/` as `*.testing.ts`. This layout assumes NgRx Signals (`@ngrx/signals` + `withEntities`), which is the direction some client contexts in this repo have taken — check whether the client you're generating for actually uses it (and whether `@ngrx/signals` is a dependency yet) before adopting the folder shape.
