...
11. **Lo que NO hay que construir en la fase 1:** resolución de conflictos, relojes lógicos, CRDTs, y drenaje por Background Sync. Los conflictos no existen sin servidor y sin segundo dispositivo. Y `SyncManager` no sólo **no es Baseline** —falta en Firefox y Safari— sino que **la posición oficial de Mozilla es `negative`**, por razones de privacidad, no de calendario. El drenaje real será código de la app en primer plano, coordinado entre pestañas con la **Web Locks API** (Baseline desde marzo de 2022, cuyo ejemplo canónico en MDN es exactamente este caso). Se deja **comentado en el código** cuál sería la implementación completa, según el criterio de recortes del proyecto.
...
**Y una nota de seguridad para el día que haya backend**, que tiene nombre propio: **OWASP API1:2023, _Broken Object Level Authorization_** `[DOCS]` — _"Attackers can exploit API endpoints that are vulnerable to broken object-level authorization by manipulating the ID of an object that is sent within the request."_ Su prevención es la regla a grabar: comprobar la autorización _"in every function that uses an input from the client to access a record in the database"_. Es decir: **el ID nunca es la comprobación de acceso**; el servidor lo acota al tenant autenticado y valida la propiedad en cada operación.

> Hay una tensión que conviene reconocer en vez de esconder: OWASP prefiere identificadores **impredecibles**, y un UUIDv7 filtra su instante de creación a quien lo tenga. El propio RFC 9562 §8 la mide y la desestima —_"Timestamps embedded in the UUID do pose a very small attack surface"_— y advierte por separado que los UUID _"MUST NOT be used as security capabilities (identifiers whose mere possession grants access)"_. Bajo la regla de arriba, la filtración se reduce a "esta orden se creó un martes", para alguien que ya tiene la orden. Aceptable. Si algún identificador llegara a ser un secreto compartido (un enlace público de seguimiento para el cliente del taller, por ejemplo), **ese** debe ser v4 aleatorio, no v7.
...
**Drenaje: Background Sync no es la respuesta, y el motivo es más fuerte que "no está implementado".** MDN lo marca como _"Limited availability… not Baseline"_ y su especificación es un borrador de la WICG, _"not a W3C Standard nor is it on the W3C Standards Track"_. Los datos de compatibilidad de MDN dan Chrome/Edge/Opera 49, y **no** Firefox, **no** Safari, **no** WebView Android. Y el dato que lo cierra: la **posición oficial de Mozilla es `negative`** `[CÓDIGO]` (`mozilla/standards-positions`, `activities.yml`, `id: background-sync`, `issue: 173`), con esta razón textual:

> "We're concerned that this feature would allow users to be tracked across networks (leaking private information about location and IP address and how they change over time), and that it would allow script execution and resource consumption when it isn't clear to the user that they're interacting with the site."

No es "todavía no llegó": es una objeción de diseño. **Conclusión de planificación: el drenaje debe ser código de la app en primer plano** (al arrancar, al evento `online`, al recuperar visibilidad, y con un temporizador), y `SyncManager` es como mucho una mejora progresiva en Chromium.

**Dos realidades del navegador que la cola tiene que manejar, y que se descubren tarde si no se anotan ahora:**

1. **Varias pestañas drenando a la vez.** La **Web Locks API** lo resuelve y es **Baseline desde marzo de 2022**; el ejemplo motivador de MDN es literalmente este caso `[DOCS]` ([MDN Web Locks](https://developer.mozilla.org/en-US/docs/Web/API/Web_Locks_API)): _"if a web app running in multiple tabs wants to ensure that **only one tab is syncing data between the network and Indexed DB**, each tab could try to acquire a `my_net_db_sync` lock, but only one tab will succeed (the leader election pattern)."_ Es `navigator.locks.request('sync', {mode:'exclusive'}, cb)` y se libera sola al terminar el callback.
2. **`navigator.onLine` miente.** MDN es explícito `[DOCS]`: _"this property is inherently unreliable, and **you should not disable features based on the online status**, only provide hints when the user may seem offline."_ Estar en una LAN sin salida a internet cuenta como "online". El disparador de drenaje puede usarlo como pista, pero el éxito real se determina intentando.

**Reintentos: backoff exponencial con jitter.** Fuente primaria localizada `[DOCS]` ([AWS Architecture Blog, Marc Brooker](https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/)), con las fórmulas textuales:

```
Sin jitter:  sleep = min(cap, base * 2 ** intento)
Full Jitter: sleep = random(0, min(cap, base * 2 ** intento))
Decorrelated: sleep = min(cap, random(base, sleep * 3))
```

Los resultados medidos de AWS: las variantes sin aleatorizar rinden claramente peor; **Full Jitter es la que menos trabajo total exige del cliente**; con 100 clientes en contienda, Full y Decorrelated **reducen las llamadas a más de la mitad**. La conclusión del artículo, textual: _"The return on implementation complexity of using jittered backoff is huge, and it should be considered a standard approach for remote clients."_ Añadir un tope de intentos y un estado de "muerta", para que una operación envenenada no bloquee la cola detrás de ella.
...
> **Por qué el tombstone es innegociable:** offline, **un borrado tiene que ser un registro, no una ausencia**. Si se borra la fila y nada más, al sincronizar el servidor no puede distinguir "esto lo borré" de "esto todavía no lo recibí", y el registro **resucita**. Cassandra documenta el mecanismo exacto y le puso nombre —_zombie_— `[DOCS]` ([tombstones](https://cassandra.apache.org/doc/latest/cassandra/managing/operating/compaction/tombstones.html)): si una réplica no recibe el tombstone a tiempo y el resto ya lo purgó, _"Cassandra treats the object on the recovered node as new data, and propagates it to the rest of the cluster"_. Dynamo lo dice en una línea: _"deleted items can resurface"_.
>
> Y de ahí sale una regla de retención que casi nadie calcula: Cassandra usa `gc_grace_seconds`, **por defecto 864 000 s (diez días)**, _"to give unresponsive nodes time to recover and process tombstones normally"_ — pero _"if the node does not recover until after the grace period ends, Cassandra may miss the deletion"_. **El período de retención del tombstone y la ventana máxima que un dispositivo puede pasar offline son el mismo número**, y en un taller ese número tiene que sobrevivir a las vacaciones de un técnico. Purgar antes no es limpieza: es un bug de corrección.
>
> Convertir un borrado físico en lógico **después** es especialmente caro porque no hay nada que migrar: la información ya no existe. Firestore, como referencia de implementación, funciona igual que lo aquí propuesto: cachea local, encola escrituras y sincroniza al volver `[DOCS]`.

**Sobre los relojes, que es donde más gente se equivoca.** `actualizadoEn` con la hora del cliente sirve para **mostrar** y para ordenar localmente, pero **no como árbitro de conflictos**. La evidencia es contundente y de primera mano:

- **W3C High Resolution Time** `[SPEC]` ([hr-time-3 §2.1](https://www.w3.org/TR/hr-time-3/)): el reloj de pared _"can decrease, making it unreliable for performance measurement **or recording the orders of events**"_.
- **NTP no lo salva**: ante un desfase entre 125 ms y 1000 s, NTP **escalona** el reloj — salta hacia adelante o hacia atrás de golpe. Y el usuario puede cambiarlo a mano.
- **Google Spanner** `[DOCS]` necesitó relojes atómicos y GPS para que el orden por tiempo fuera seguro entre máquinas; su propia documentación remata el argumento desde el otro lado: _"Achieving proper timestamping is **trivial for a single-machine database** (for example, you can just assign timestamps from a global, monotonically increasing counter)."_

**Consecuencia directa para el outbox:** el orden de reenvío **no** puede salir de `Date.now()`. De ahí el `++secuencia` autoincremental de §9 — un contador persistido e incrementado dentro de la misma transacción que la escritura es monótono por construcción, sobrevive a las recargas y no depende de ningún reloj.

**Y sobre LWW, que es la tentación por defecto:** es una política de pérdida de datos documentada, no un algoritmo. Kleppmann lo anota sin rodeos en sus apuntes de Cambridge `[DOCS]` — _"Last writer wins (LWW)… **Note: data loss!**"_ — con el ejemplo exacto de este dominio: si en un dispositivo se edita la hora de un evento y concurrentemente en otro se edita el título, _"the update of the time is applied to both devices, while **the update of the title is discarded**"_. Riak lo acota con precisión: LWW _"will necessarily drop some writes in the case of concurrent updates"_, y es seguro _"**if you are certain that there will be no concurrent updates**"_. Un taller donde el técnico edita la orden en la tablet mientras el mostrador edita la misma orden no cumple esa condición.

La forma estándar y ya especificada de detectar escrituras a ciegas es el control de concurrencia optimista de HTTP: `ETag` + `If-Match`, con **412 Precondition Failed** `[SPEC]` (RFC 9110 §8.8.3, §13.1.1 y §15.5.13). La spec lo nombra literalmente: `If-Match` sirve _"to prevent accidental overwrites when multiple user agents might be acting in parallel on the same resource (i.e., **to prevent the 'lost update' problem**)"_. La diferencia con LWW es la que importa: **al perdedor se le avisa**, no se le descarta en silencio. Dejar un campo `version` por fila desde hoy es gratis y evita la migración después.

> **La mejora más barata de todas, y no requiere nada del backend:** enviar el **cambio de campos**, no el objeto entero. Dexie Cloud lo formula así `[DOCS]`: _"Update-operations manipulate individual properties rather than replacing entire objects. **Update operations prohibit conflicts when two different clients mutate different properties on the same object.**"_ Cassandra hace lo mismo con marcas de tiempo por columna. Dos personas editando campos distintos de la misma orden dejan de ser un conflicto. Esto sólo funciona si la capa de datos guarda diffs — decidirlo después es reescribir todos los repositorios.

**Relojes lógicos: por qué no hacen falta hoy.** Se verificaron las fuentes, y la conclusión es que ninguna resuelve el problema de la fase 1:

- Un **reloj de Lamport** (Lamport 1978, CACM 21(7)) da orden total, pero el propio paper advierte que la condición inversa no se cumple: _"we cannot expect the converse condition to hold as well"_. Es decir, **no puede detectar concurrencia** — sólo impone un orden arbitrario, que es exactamente cómo se deriva LWW.
- Los **Hybrid Logical Clocks** (Kulkarni, Demirbas et al., 2014) sí capturan causalidad manteniéndose cerca del reloj NTP y **caben en 64 bits**, pero siguen sin detectar el conflicto por sí solos.
- Los **vector clocks** sí lo detectan, pero su tamaño crece con el número de actores, y en un navegador cada sesión tiende a generar una identidad nueva — el propio Yjs advierte que reutilizar un `ClientID` puede corromper el documento _"without a way to recover"_.

**Ninguno es necesario mientras haya un solo dispositivo y ningún servidor.** Se anotan porque la decisión de guardar `version` + `actualizadoEn` + `secuencia` desde el día uno es justamente lo que deja la puerta abierta a cualquiera de los tres.
...
- **CRDTs** (Automerge, Yjs). Resuelven fusión automática sin coordinación, pero tienen un límite **demostrado**, no opinable. El informe de referencia de Shapiro, Preguiça, Baquero y Zawirski (INRIA RR-7506) `[DOCS]` lo prueba con el contador no negativo: no basta con que cada réplica se niegue a decrementar en cero, porque _"two replicas at value 1 might still concurrently decrement, and the value converges to −1"_. Y lo generaliza: _"**In general, maintaining a particular shape, such as a tree or a DAG, cannot be done by a CRDT. Such a global invariant cannot be determined locally; maintaining it requires synchronisation.**"_ La consistencia eventual fuerte promete **convergencia, no validez**. Una orden de trabajo tiene invariantes (folio único, total que cuadra, una placa vigente por vehículo); un documento colaborativo no. Añádase el coste que los propios autores del ensayo _Local-first software_ reconocen —_"CRDTs accumulate a large change history, which creates performance problems"_, y que ese historial _"can't easily be truncated"_— y queda claro que no es el problema de esta app. Lo que sí encaja, y es la postura de Dexie Cloud, es reservarlos para **campos de texto libre** (un `diagnostico`) conviviendo con un modelo servidor-autoritativo para el resto: _"Both models can coexist in the same application."_
...
**La forma recomendada es exactamente la que el mapa propone**, y hay fuente primaria `[DOCS]` ([Azure Architecture Center, _Architectural Approaches for Storage and Data in Multitenant Solutions_](https://learn.microsoft.com/en-us/azure/architecture/guide/multitenant/approaches/storage-data)). En su lista de **antipatrones**: _"When you work within a single database, avoid creating individual tables for each tenant… **Instead, consider using a single set of multitenant tables with a tenant identifier column.**"_ La misma página advierte contra otros dos vicios relevantes aquí: personalizar el esquema para un solo tenant, y —importante para una base local que no se puede forzar a migrar— _"ensure that your application version is **backwards-compatible with at least one previous schema version**, and sequence destructive schema changes across multiple versions to support rollbacks."_

Y sobre por qué esto no se pospone, el AWS Well-Architected SaaS Lens `[DOCS]` es tajante: _"Tenant isolation is one of the foundational topics that every SaaS provider must address… Crossing this boundary in any form would represent a **significant and potentially unrecoverable event** for a SaaS business."_

Lo que sí hay que pensar bien es el **orden dentro del índice compuesto**: `[tallerId+placa]`, no `[placa+tallerId]`. Dexie documenta que _"Only the leading parts of a compound index can be used alone - never the trailing parts"_, y PostgreSQL formula la misma regla del lado servidor: _"the index is most efficient when there are constraints on the **leading (leftmost) columns**"_ `[DOCS]`. Liderar por tenant permite además recorrer todo el taller con el mismo índice. Cambiar el orden después es una migración de esquema, así que se elige una vez.

> **El argumento decisivo, en concreto.** Hoy `SEED_TALLER` es el único taller, así que estampar `tallerId` en cada fila es **un backfill con una constante** dentro de un bloque `version(n)`. El día que exista un segundo taller, **no hay valor correcto que rellenar** para los `Cliente`, `Vehiculo` y `Factura` ya creados: la información de a qué taller pertenecían nunca se guardó. Esa asimetría —hoy trivial, mañana imposible— es todo el argumento.
...
Lo que hace especial este contexto: **no se puede forzar el refresco de la base de un usuario**. Cada migración debe funcionar contra **cualquier** versión anterior que exista en cualquier navegador, sin supervisión y sin posibilidad de rollback. Azure lo formula como regla operativa `[DOCS]`: la aplicación debe ser _"backwards-compatible with at least one previous schema version"_, y los cambios destructivos deben repartirse en varias versiones.

Dos detalles mecánicos que se pagan si se ignoran:

- **Otra pestaña puede bloquear la migración indefinidamente.** IndexedDB emite `blocked` cuando una conexión antigua impide la transacción `versionchange`; `idb` expone `blocked`, `blocking` y `terminated` como callbacks. **Hay que cablearlos desde el principio** — en `apps/bahia` hoy no lo están, y el síntoma es una app que se queda colgada al abrir tras una actualización.
- **Dexie multiplica por 10 el número de versión.** Su documentación lo dice literal: _"the backend DB will get the given version number multiplied by 10"_. Para Bitácora, que empieza de cero, es irrelevante. Importa sólo si alguna vez se migrara una base existente de `idb` a Dexie: `version(1)` de Dexie es la versión 10 del backend, que dispararía un upgrade sobre cualquier base preexistente en versión 2. Esto tiene una consecuencia inmediata para el proyecto:
...
| **`tallerId` sólo existe en `Usuario` y `OrdenTrabajo`.** `Cliente`, `Vehiculo` y `Factura` no lo tienen.                  | Verificado. Es justo la asimetría de §4.5: hoy se rellena con una constante, mañana no hay valor correcto.                                                                        |
| **`siguienteNumeroOrden()` deriva `OT-0007` del `max()` de las órdenes presentes localmente.**                             | **Es una llave natural secuencial generada en cliente.** Dos dispositivos offline acuñan el mismo folio; dos talleres también. Y el número **se imprime y se dice por teléfono**. |
| **`update()` hace `get` y luego `put` en dos transacciones separadas.**                                                    | Es una actualización perdida **hoy, en local, sin red**: dos pestañas se pisan. Y es exactamente el sitio donde el outbox tendría que escribirse atómicamente.                    |
...
> **El hallazgo más caro de los siete es el folio.** El comentario del código explica la elección —_"no un contador aparte, para no desincronizarse"_— y el razonamiento es correcto **para un solo dispositivo sin red**, que es lo que Bahía es. Deja de serlo en cuanto hay dos. RFC 9562 §6.13 describe el patrón general del error: _"A common issue observed in database schema design is the assumption that a particular value will never change, which later turns out to be an incorrect assumption."_ Para Bitácora la lección es concreta: **el folio es un atributo de presentación, no una identidad**, y debe ser asignado por el servidor o llevar el taller y el año en su composición. Lo barato es decidirlo antes de que un cliente tenga una factura impresa con ese número.
...
| --- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 0   | **Folio secuencial (`OT-0007`) generado en cliente como llave natural**                  | La única decisión de la lista cuyo error **sale del sistema**: el número va impreso en una factura que el cliente ya tiene. Dos dispositivos offline, o dos talleres, acuñan el mismo. Arreglarlo es renumerar papeles ajenos | **El folio es presentación, nunca identidad.** Que lo asigne el servidor, o componerlo con taller y año |
...
| **Redacción exacta del `VersionError` al abrir IndexedDB con una versión menor**  | `[NO CONFIRMADO]`. La sección del algoritmo no se pudo renderizar. Creencia firme y consistente con la práctica, pero **sin cita** en este documento. No afecta la decisión.                                                                                     |
| **Postura oficial de Supabase sobre offline**                                     | **No se encontró ninguna.** Su guía de _local development_ trata del CLI y Docker para desarrolladores, **no** de sincronización offline del usuario final. Anotado para que nadie la cite por error; la ruta real sería PowerSync o Electric sobre su Postgres. |
...
| IETF — `Idempotency-Key` (draft-07, **expirado**)         | <https://datatracker.ietf.org/doc/draft-ietf-httpapi-idempotency-key-header/>                     |
| AWS — Exponential Backoff And Jitter                      | <https://aws.amazon.com/blogs/architecture/exponential-backoff-and-jitter/>                       |
| AWS SQS — entrega _al menos una vez_                      | <https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/standard-queues.html> |
| Mozilla standards-positions — Background Sync `negative`  | <https://github.com/mozilla/standards-positions/blob/main/activities.yml>                         |
| MDN — Web Locks API (elección de líder entre pestañas)    | <https://developer.mozilla.org/en-US/docs/Web/API/Web_Locks_API>                                  |
| MDN — `navigator.onLine` no es fiable                     | <https://developer.mozilla.org/en-US/docs/Web/API/Navigator/onLine>                               |
| W3C High Resolution Time (reloj de pared no ordena)       | <https://www.w3.org/TR/hr-time-3/>                                                                |
| Lamport 1978 — _Time, Clocks, and the Ordering of Events_ | <https://lamport.azurewebsites.net/pubs/time-clocks.pdf>                                          |
| Hybrid Logical Clocks (Kulkarni, Demirbas et al., 2014)   | <https://cse.buffalo.edu/tech-reports/2014-04.pdf>                                                |
| Kleppmann — apuntes de sistemas distribuidos (LWW)        | <https://www.cl.cam.ac.uk/teaching/2122/ConcDisSys/dist-sys-notes.pdf>                            |
| Riak — resolución de conflictos y riesgo de LWW           | <https://docs.riak.com/riak/kv/latest/developing/usage/conflict-resolution/index.html>            |
| Cassandra — tombstones, zombies y `gc_grace_seconds`      | <https://cassandra.apache.org/doc/latest/cassandra/managing/operating/compaction/tombstones.html> |
| Google Spanner — TrueTime y consistencia externa          | <https://cloud.google.com/spanner/docs/true-time-external-consistency>                            |
| Shapiro et al. — CRDTs (INRIA RR-7506)                    | <https://inria.hal.science/inria-00555588/document>                                               |
| Ink & Switch — Local-first software                       | <https://www.inkandswitch.com/essay/local-first/>                                                 |
| CouchDB — protocolo de replicación                        | <https://docs.couchdb.org/en/stable/replication/protocol.html>                                    |
...
| PostgreSQL — índices multicolumna (regla de la izquierda) | <https://www.postgresql.org/docs/current/indexes-multicolumn.html>                                |
| Azure — datos en soluciones multitenant                   | <https://learn.microsoft.com/en-us/azure/architecture/guide/multitenant/approaches/storage-data>  |
| AWS Well-Architected SaaS Lens — aislamiento de tenant    | <https://docs.aws.amazon.com/wellarchitected/latest/saas-lens/tenant-isolation.html>              |
